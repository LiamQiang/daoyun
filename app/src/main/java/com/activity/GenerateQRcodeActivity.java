...
public class GenerateQRcodeActivity extends Activity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        content = getIntent().getExtras().getString("id");
        ...
        qrcode_bitmap = QRCodeUtil.createQRCodeBitmap(content, 650, 650, "UTF-8",
    ...
    private void saveImg(Bitmap bitmap){
        ...
        boolean isSaveSuccess = ImageUtil.saveImageToGallery(GenerateQRcodeActivity.this, bitmap,fileName);
        if (isSaveSuccess) {
            Toast.makeText(GenerateQRcodeActivity.this, "图片已保存至本地", Toast.LENGTH_LONG).show();
        } else {
            Toast.makeText(GenerateQRcodeActivity.this, "保存图片失败，请稍后重试", Toast.LENGTH_SHORT).show();
        }
    }
    ...
    private void generateQrcodeAndDisplay() {
        ...
        qrcode_bitmap = QRCodeUtil.createQRCodeBitmap(content, 800, 800, "UTF-8",
                "H", "1", color_black, color_white);
        iv_qrcode.setImageBitmap(qrcode_bitmap);
    }
    ...
    private void imgChooseDialog(){
        AlertDialog.Builder choiceBuilder = new AlertDialog.Builder(GenerateQRcodeActivity.this);
        choiceBuilder.setCancelable(false);
        choiceBuilder
                .setTitle("选择")
                .setSingleChoiceItems(new String[]{"存储至手机"}, -1,
                        new DialogInterface.OnClickListener() {
                            @Override
                            public void onClick(DialogInterface dialog, int which) {
                                switch (which) {
    ...
    private void takePhoto() {
        ...
        if (Build.VERSION.SDK_INT < 24) {
            ...
            imageUri = FileProvider.getUriForFile(GenerateQRcodeActivity.this, "com.example.xch.generateqrcode.fileprovider", outputImage);
        }
        // 启动相机程序
    ...
    private void choosePhotoFromAlbum() {
        if (ContextCompat.checkSelfPermission(GenerateQRcodeActivity.this, Manifest.permission.WRITE_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED) {
