...
public class MyItemActivity extends BaseActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        initUI();
        sendRequest();
        ((TextView)findViewById(R.id.tvMyBar)).setText("在售商品");
    }

    private void sendRequest(){
        //HttpUtils.get(APIInterface.MY_ITEM+"?account="+MyApplication.getUser().getUsername()+"&p=1",responseHandler);
    }
   ...
   /* JsonHttpResponseHandler responseHandler= new JsonHttpResponseHandler(){
        ...
        public void onSuccess(int statusCode, Header[] headers, JSONObject response) {
            ...
            listView.setAdapter(new CommonBaseAdapter<Item>(MyItemActivity.this,mcommomList.getList(),R.layout.sub_home_list) {
                @Override
                protected void convert(CommonViewHolder viewHolder, Item item) {
                    ...
                    viewHolder.getView(R.id.tvTest).setOnClickListener(v -> {
                        Intent intent=new Intent(MyItemActivity.this, ItemDetailActivity.class);
                    ...
                    viewHolder.getView(R.id.tvCollections).setOnClickListener(v -> {
                        if (MyApplication.getUser().getUid()!=0){//判断是否登录
                                    ...
                                    try {
                                        if (response.getInt("code")==200){
                                            if (response.getInt("data")==1){
                                                Toast.makeText(MyItemActivity.this,"收藏成功", Toast.LENGTH_SHORT).show();
                                                tv.setCompoundDrawablesWithIntrinsicBounds(R.mipmap.love_red,0,0,0);
                                                tv.setText(String.valueOf(Integer.parseInt(tv.getText().toString())+1));
                                            }else {
                                                Toast.makeText(MyItemActivity.this,"取消收藏成功", Toast.LENGTH_SHORT).show();
                            ...
                            startActivity(new Intent(MyItemActivity.this, LoginActivity.class));
                        }
                    });
                }
            });
