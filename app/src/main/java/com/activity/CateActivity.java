...
public class CateActivity extends Activity {
    ...
    private void initUI(){
        ...
        ((TextView)findViewById(R.id.tvMyBar)).setText("分类");
        cateId=getIntent().getStringExtra("cate");
        switch (cateId){
            case "1":
                ((TextView)findViewById(R.id.tvMyBar)).setText("数码产品");
                break;
            case "3":
                ((TextView)findViewById(R.id.tvMyBar)).setText("书籍");
                break;
            case "2":
                ((TextView)findViewById(R.id.tvMyBar)).setText("服饰");
                break;
            case "4":
                ((TextView)findViewById(R.id.tvMyBar)).setText("其他");
                break;
        }
    }

    private void sendRequest(){
        HttpUtils.get(APIInterface.HOME+"?cid="+ cateId ,responseHandler);
    }
            ...
            listView.setAdapter(new CommonBaseAdapter<Item>(CateActivity.this,mcommomList.getList(),R.layout.sub_home_list) {
                ...
                protected void convert(CommonViewHolder viewHolder, Item item) {
                    ...
                    viewHolder.getView(R.id.tvTest).setOnClickListener(v -> {
                        Intent intent=new Intent(CateActivity.this, ItemDetailActivity.class);
                    ...
                    viewHolder.getView(R.id.tvCollections).setOnClickListener(v -> {
                        if (MyApplication.getUser().getUid()!=0){//判断是否登录
                                    ...
                                    try {
                                        if (response.getInt("code")==200){
                                            if (response.getInt("data")==1){
                                                Toast.makeText(CateActivity.this,"收藏成功", Toast.LENGTH_SHORT).show();
                                                tv.setCompoundDrawablesWithIntrinsicBounds(R.mipmap.love_red,0,0,0);
                                                tv.setText(String.valueOf(Integer.parseInt(tv.getText().toString())+1));
                                            }else {
                                                Toast.makeText(CateActivity.this,"取消收藏成功", Toast.LENGTH_SHORT).show();
                            ...
                            startActivity(new Intent(CateActivity.this, LoginActivity.class));
                        }
                    });
                }
            });
